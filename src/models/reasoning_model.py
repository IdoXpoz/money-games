...
class ReasoningModel:
    ...
    def run(self, prompt: str, custom_generation_params: dict = None) -> str:
        """
        Run prompt through the reasoning model and extract the final answer.

        Args:
            prompt (str): The input prompt
            custom_generation_params (dict): Optional custom generation parameters
    ...
    def run_reasoning_inference_and_split_thinking_content(self, prompt: str, custom_generation_params: dict = None):
        # Use custom params if provided, otherwise use default config
        generation_params = custom_generation_params or REASONING_GENERATION_PARAMS
